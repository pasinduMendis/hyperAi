...
exports.getAutoTrackJourneyByWebsiteIdWithFilterWithButtonsShare = async (req, res) => {
  ...
  try {
    ...
    if(existingWebUser.Items[0].shareAllowEmailInvitations && existingWebUser.Items[0].shareAllowEmailInvitations.length>0){
      for(let i=0;i<existingWebUser.Items[0].shareAllowEmailInvitations.length;i++){
        if(userEmail !="" && existingWebUser.Items[0].shareAllowEmailInvitations[i].emails.includes(userEmail)){
          isInvitation=true
        }
      }
    }

    if(existingWebUser.Items[0].shareAllow != true && isInvitation==false){
        return res.status(400).send({
...
for(let i=0;i<resResitl.length;i++){
  ...
  finalResultRes.push({result:returnResult,dateRange:i==0?`until ${filterEnd}`:i==dateList.length+1?`${filterStart} to present`:`${filterStart} to ${filterEnd}`})
    ...
    return res.status(200).send({ userJourney: finalResultRes,website: existingWebUser.Items[0]});
