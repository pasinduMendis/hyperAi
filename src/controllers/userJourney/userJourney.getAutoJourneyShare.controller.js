...
exports.getAutoTrackJourneyByWebsiteId = async (req, res) => {
  ...
  try {
    ...
    if(userEmail && userEmail !=null && existingWebUser.Items[0].shareAllowEmailInvitations && existingWebUser.Items[0].shareAllowEmailInvitations.length>0){
      for(let i=0;i<existingWebUser.Items[0].shareAllowEmailInvitations.length;i++){
        if(existingWebUser.Items[0].shareAllowEmailInvitations[i].emails.includes(userEmail)){
          isInvitation=true
        }
      }
    }
